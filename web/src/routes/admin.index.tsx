/** 路由 `/admin` 概览：全站资源总量 + 近 30 天调用量/注册量。 */
  ...
  return (
    <Card>
      <CardContent class="p-4">
        ...
        <Show when={props.sub}>
          <div class="text-xs text-muted-foreground">{props.sub}</div>
        </Show>
      </CardContent>
    </Card>
  );
...
function Overview() {
  const [stats] = createResource(() => api.admin.stats.overview());
  const [series] = createResource(() => api.admin.stats.timeseries(30));
      ...
      <div class="grid gap-4 lg:grid-cols-2">
        <Card>
          <CardContent class="space-y-2 p-4">
            <div class="text-sm font-medium">近 30 天 API 调用量</div>
            <Show when={series()} fallback={<div class="h-28" />}>
        ...
        <Card>
          <CardContent class="space-y-2 p-4">
            <div class="text-sm font-medium">近 30 天注册量</div>
            <Show when={series()} fallback={<div class="h-28" />}>
