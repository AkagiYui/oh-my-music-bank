...
「曲库管理 → 哔哩哔哩导入」的裁剪条下方有一条独立的识别选段条：整条宽度是当前裁剪区间，
在其中再拖出真正送检的片段。讯飞最长 58 秒（其接口 2MB 上限约合 65 秒），
网易云固定 6 秒（其指纹算法只接受 8kHz 单声道的 48000 个样点），窗口默认落在裁剪范围正中。

讯飞需要在「系统管理 → 集成配置」填 AppID 与 APIKey。网易云不需要账号，
但指纹算法只存在于网易官方 Chrome 扩展「云音乐听歌」的 `afp.wasm` 中，本仓库不分发该文件：
在集成配置点击「从 Chrome 应用店拉取」，服务端下载官方签名包并存入私有桶。

应用店只提供扩展的当前版本，没有按版本号取包的接口，所以这里锁的是**内容哈希**而不是版本号：
`afp.wasm` 与 `sandbox.bundle.js` 各自比对已审计的 SHA-256，任一不符就整体失败并报出实际哈希，
不会静默换掉在管理员浏览器里执行的代码。校验发生在写入之前，失败时已保存的旧副本保持可用，
识别功能不受影响。扩展确实更新时，需要人工复核新版本后更新
`internal/service/recognize/afp.go` 里的两个常量再重新拉取。

集成配置里可以关闭「校验内容哈希」，也可以填自定义下载地址（CRX 或 ZIP，走 `safefetch` 校验）。
两者都是给上游更新后应急和自建镜像用的：关闭校验意味着拉进来的是未经审计、且要在管理员浏览器中
执行的第三方代码，界面上会持续标注该副本「未与已审计版本比对」。

资源也可以在构建镜像时预置：`ommb fetch-netease-afp --out <目录>` 与管理端共用同一套解包和校验逻辑，
Containerfile 的 `NETEASE_AFP=1`（默认开启，`NETEASE_AFP_URL` 可指定自定义地址）会把两个文件放进
`/opt/ommb/netease-afp`，运行时目录由 `recognize.netease_afp_dir` 指定。管理员拉取的副本优先，
镜像内预置的作为回落。**注意：这两个文件是网易官方扩展的一部分，把它们打进对外分发的镜像等于代为
分发第三方版权二进制**，公开发布镜像前请自行确认合规，或用仓库变量 `BUNDLE_NETEASE_AFP=0` 关闭。
预置失败（例如上游更新导致校验不过）只在构建日志里告警，不阻断镜像发布，管理员仍可在运行时拉取。
识别时由管理员浏览器取回该文件，在 Worker 内算出指纹回传，服务端仅转发匹配请求。
该 wasm 的导入表只有 embind 类型注册与 abort/memcpy/环境变量等运行时函数，
没有网络、存储、DOM 或文件系统通道，也没有 `EM_ASM`，只做纯计算。
